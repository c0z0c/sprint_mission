...
```mermaid
graph LR
    A["Input 300×300"] --> B["Feature Map 1<br/>38×38"]
    A --> C["Feature Map 2<br/>19×19"]
    A --> D["Feature Map 3<br/>10×10"]
    A --> E["Feature Map 4<br/>5×5"]
    A --> F["Feature Map 5<br/>3×3"]
    A --> G["Feature Map 6<br/>1×1"]
```
...
```mermaid
graph TD
    subgraph "SSD300_VGG16"
        A1["Pre-trained COCO Model"] --> A2["VGG16 Backbone"]
        A2 --> A3["6 Feature Maps"]
        A3 --> A4["Classification Head (교체)"]
        A3 --> A5["Regression Head (유지)"]
    end
```

```mermaid
graph TD
    subgraph "SSD300_ResNet34"
        B1["Pre-trained ImageNet ResNet34"] --> B2["Custom SSD Backbone"]
        B2 --> B3["7 Feature Maps"]
        B3 --> B4["새로운 Classification Head"]
        B3 --> B5["새로운 Regression Head"]
    end
```
