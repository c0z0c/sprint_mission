...
class QuantizedModelWrapper:
        ...
        with torch.no_grad():
            for img_path in sources:
                try:
                    ...
                    # 모델 타입에 따라 추론 실행
                    if self.model_type == 'onnx':
                        # ONNX Runtime 추론 (dtype 명시)
                        img_np = img_tensor.unsqueeze(0).cpu().numpy().astype(np.float32)
                        
                        # 첫 번째 이미지만 상세 로그 출력
                        if len(results) == 0:
                            logger.info(f"[ONNX] 입력 shape={img_np.shape}, dtype={img_np.dtype}, 범위=[{img_np.min():.3f}, {img_np.max():.3f}]")
                        ...
                        if len(results) == 0:
                            logger.info(f"[ONNX] 원본 출력 shape={onnx_output.shape}, dtype={onnx_output.dtype}")
                        
                        # YOLOv8 ONNX 출력 형식 자동 감지 및 전치
                        # 일부 모델은 (batch, features, boxes) 형식으로 출력 → (batch, boxes, features)로 전치 필요
                        if len(onnx_output.shape) == 3:
                            batch, dim1, dim2 = onnx_output.shape
                            # features 차원이 더 작으면 (batch, features, boxes) 형식
                            if dim1 < dim2 and dim1 < 100:  # features는 보통 5~85 범위
                                if len(results) == 0:
                                    logger.info(f"[ONNX] 출력 전치 필요: {onnx_output.shape} → ({batch}, {dim2}, {dim1})")
                                onnx_output = np.transpose(onnx_output, (0, 2, 1))
                                if len(results) == 0:
                                    logger.info(f"[ONNX] 전치 후 shape={onnx_output.shape}")
                        
                        if len(results) == 0:
                            logger.info(f"[ONNX] 최종 출력 shape={onnx_output.shape}")
                            logger.info(f"[ONNX] 출력 범위=[{onnx_output.min():.6f}, {onnx_output.max():.6f}]")
                            if len(onnx_output.shape) == 3 and onnx_output.shape[2] > 4:
                                conf_col = onnx_output[:, :, 4]
                                logger.info(f"[ONNX] Confidence 범위=[{conf_col.min():.6f}, {conf_col.max():.6f}]")
                                logger.info(f"[ONNX] Confidence > 0.001 개수: {(conf_col > 0.001).sum()}/{conf_col.size}")
                        
                        predictions = torch.from_numpy(onnx_output).to(self.device)
                    else:
                        # PyTorch 추론
                    ...
                    # NMS 전 로그 (첫 번째 이미지만)
                    if len(results) == 0 and self.model_type == 'onnx':
                        logger.info(f"[ONNX] NMS 전 predictions shape={predictions.shape}, dtype={predictions.dtype}")
                        if len(predictions.shape) == 3:
                            conf_tensor = predictions[:, :, 4]
                            logger.info(f"[ONNX] NMS 전 Confidence 범위=[{conf_tensor.min():.6f}, {conf_tensor.max():.6f}]")
                    ...
                    # NMS 후 로그 (첫 번째 이미지만)
                    if len(results) == 0 and self.model_type == 'onnx':
                        logger.info(f"[ONNX] NMS 후 detections 수={len(det)}, conf_thres={conf}, iou_thres={iou}")
                        if len(det) > 0:
                            logger.info(f"[ONNX] 첫 번째 박스 예시: {det[0]}")
    ...
    def _preprocess_image(self, img, imgsz=640):
        ...
        try:
            ...
            # 레터박스 리사이즈 (auto=False: 고정 크기 640x640로 강제)
