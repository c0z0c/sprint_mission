...
class MovieEditManager:
    ...
    def _search_movies(self, title: str = "", tmdb_id: str = ""):
        ...
        try:
            ...
            response = api_get(f"{self.api_url}/movies/search", params=params)
    ...
    def _load_more_movies(self):
        ...
        try:
            ...
            if st.session_state["edit_search_mode"]:
                ...
                response = api_get(f"{self.api_url}/movies/paginated", params=params)
        ...
        with st.container(border=True):
            ...
            with cols_main[0]:
                if movie.get("poster_local_path"):
                    poster_path = f"{self.browser_api_url}/{movie['poster_local_path']}"
                else:
    ...
    def _update_movie(self, movie_id: int, update_data: dict):
        """영화 업데이트 (PATCH 방식)"""
        try:
            response = api_patch(f"{self.api_url}/movies/{movie_id}", json=update_data)
    ...
    def _delete_movie(self, movie_id: int):
        """영화 삭제"""
        try:
            response = api_delete(f"{self.api_url}/movies/{movie_id}")
