...
class MovieManager:
    ...
    def _get_max_tmdb_id(self) -> int:
        ...
        """
        try:
            response = api_get(f"{self.api_url}/movies/max-tmdb-id", timeout=(1, 3))
    ...
    def render(self):
        ...
        if st.button(
            "###### 🎬 영화 관리", key="toggle_sidebar_management", help="사이드바"
        ):
            st_sidebar_show()
        ...
        with st.form("movie_registration_form"):
            ...
            with col3:
                ...
                original_title = st.text_input(
                    "원제 (Original Title)", placeholder="예: Interstellar"
                )
            ...
            with col4:
                ...
                backdrop_path = st.text_input(
                    "배경 이미지 URL", placeholder="TMDB 배경 이미지 URL"
                )
    ...
    def _register_movie(
        ...
        try:
            response = api_post(
                f"{self.api_url}/movies/", json=movie_data, timeout=(1, 3)
            )
