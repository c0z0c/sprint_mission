...
class YOLOEvaluator:
    ...
    def _load_model(self):
        ...
        if self.model_type == 'yolo_pt':
            ...
            # ONNX 모델은 Ultralytics YOLO로 직접 로딩 (네이티브 지원)
            logger.info("ONNX 모델을 Ultralytics YOLO로 로딩")
            
            model_size_mb = os.path.getsize(self.model_path) / (1024 * 1024)
            logger.info(f"ONNX 모델 크기: {model_size_mb:.2f} MB")
            
            # Ultralytics는 ONNX를 네이티브로 지원
    ...
    def validate(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: YOLO val() 메서드의 추가 파라미터 (rect, augment 등)
        ...
        Example:
            ...
            >>> 
            >>> # rectangular inference 비활성화
            >>> evaluator.validate(rect=False, augment=True)
        """
