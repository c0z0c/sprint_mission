...
class SyncScheduler:
    ...
    async def run_initial_sync_if_needed(self) -> None:
        ...
        if popular_enabled:
            ...
            try:
                await self._sync_popular_movies(max_pages)
    ...
    async def _sync_popular_movies(self, max_pages: int) -> None:
        """인기 영화 동기화 실행"""
    ...
    async def _download_posters_for_movies(self, movies: list) -> None:
        """영화 목록에 대한 포스터 다운로드"""
        import aiohttp
